...
ListWidget::ListWidget(TextRenderer* _text_renderer, int _x, int _y,
...
void ListWidget::Render() {
    ...
    for (ListWidgetItem list_item : list_items) {
        if (render_item_index == item_index)
            current_color = highlighted_color;
        else
            current_color = normal_color;

        RenderedTextMetrics rtm = text_renderer->Render(font_name, x, y + y_cursor,
                                                        list_item.text, current_color);
