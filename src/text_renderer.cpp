...
TextRenderer::TextRenderer(SDL_Renderer *_renderer, Config *_config,
                           Data *_data) {
    renderer = _renderer;
    config = _config;
    data = _data;
    ...
    for (const auto &kv : data->GetFonts()) {
        ...
        for (char &ch : CHARACTERS) {
            SDL_Surface *char_surface =
                TTF_RenderGlyph_Blended(kv.second, ch, white);
            ...
            TTF_GlyphMetrics(kv.second, ch, &minx, &maxx, &miny, &maxy,
                             &advance);
            line_height = TTF_FontLineSkip(kv.second);
            ascent = TTF_FontAscent(kv.second);
            descent = TTF_FontDescent(kv.second);
            temp_char_metrics_map.insert(std::make_pair(
                ch, CharacterMetrics{minx, maxx, miny, maxy, advance,
                                     char_surface->w, char_surface->h,
        ...
        char_metrics_map.insert(
            std::make_pair(kv.first, temp_char_metrics_map));

        LOG_F(INFO, "Generated {} textures for characters in font: {}",
              CHARACTERS.length(), kv.first);
    }
...
RenderedTextMetrics TextRenderer::Render(std::string font_name, int x, int y,
    ...
    while ((open_tag_position = text.find_first_of(
        ...
        size_t next_open_tag_cursor_position =
            open_tag_position + open_tag.size();
        while ((next_open_tag_position = text.find_first_of(
                    open_tag, next_open_tag_cursor_position)) !=
                   std::string::npos &&
            ...
            open_tag_position = next_open_tag_position;
            next_open_tag_cursor_position =
                next_open_tag_position + open_tag.size();
        }

        std::string tag_str = text.substr(
            open_tag_position + open_tag.size(),
        ...
        if (tag_map.find(tag_cursor_position) != tag_map.end())
            ...
            tag_map.insert(
                std::make_pair(tag_cursor_position, temp_tag_vector));
        }
    ...
    for (unsigned int i = 0; i < text.size(); i++) {
        ...
        if (current_text_width >= current_text_max_width &&
            (wordwrap || charwrap)) {
    ...
    for (unsigned int i = 0; i < text.size(); i++) {
        ...
        for (const std::string &tag_str : tag_map[i]) {
            ...
            if (delimiter_position != std::string::npos) {
                ...
                std::string attr_value =
                    tag_str.substr(delimiter_position + 1,
                                   tag_str.size() - (delimiter_position + 1));
                ...
                if (attr_name == "color_name") {
                    ...
                    int color_delimiter_position,
                        color_delimiter_cursor_position = 0;
                    while ((color_delimiter_position = attr_value.find_first_of(
                                ',', color_delimiter_cursor_position)) !=
                           std::string::npos) {
                        std::string color_code_str = attr_value.substr(
                            color_delimiter_cursor_position,
                            color_delimiter_position -
                                color_delimiter_cursor_position);
                        if (is_number(color_code_str))
                            color_codes_vector.push_back(stoi(color_code_str));
                        color_delimiter_cursor_position =
                            color_delimiter_position + 1;
                    }
                    std::string last_color_code_str = attr_value.substr(
                        color_delimiter_cursor_position,
                        attr_value.size() - color_delimiter_cursor_position);
                    ...
                    for (auto color_code : color_codes_vector) {
                        LOG_F(INFO, "color code extracted from tag: {}",
                              color_code);
                    }
        ...
        renderedTextMetrics.height = character_metrics.line_height +
                                     line_index * character_metrics.line_height;
        ...
        char_rect.y =
            y + padding_top + line_index * character_metrics.line_height;
        char_rect.w = character_metrics.width;
        char_rect.h = character_metrics.height;

        SDL_SetTextureColorMod(char_texture, current_color.r, current_color.g,
                               current_color.b);
