    ...
    for (const auto &kv : data->GetFonts()) {
        ...
        for (char &ch : CHARACTERS) {
            ...
            temp_char_metrics_map.insert(std::make_pair(
                ch, CharacterMetrics{minx, maxx, miny, maxy, advance, char_surface->w, char_surface->h, line_height, ascent, descent}));
...
void TextRenderer::Render(std::string font_name, int x, int y, std::string text, SDL_Color color) {
    ...
    while ((open_tag_position = text.find_first_of(open_tag, tag_cursor_position)) != std::string::npos) {
        ...
        while ((next_open_tag_position = text.find_first_of(open_tag, next_open_tag_cursor_position)) != std::string::npos &&
    ...
    for (unsigned int i = 0; i < text.size(); i++) {
        ...
        if (tag_map.find(i) != tag_map.end() && !tag_map[i].empty())
            for (const std::string &tag_str : tag_map[i])
                if (tag_str == newline_tag) text_width = 0;
            ...
            if (delimiter_position != std::string::npos) {
                ...
                if (attr_name == "color_name") {
                    ...
                    while ((color_delimiter_position = attr_value.find_first_of(',', color_delimiter_cursor_position)) != std::string::npos) {
                        std::string color_code_str =
                            attr_value.substr(color_delimiter_cursor_position, color_delimiter_position - color_delimiter_cursor_position);
                    ...
                    std::string last_color_code_str =
                        attr_value.substr(color_delimiter_cursor_position, attr_value.size() - color_delimiter_cursor_position);
